...
def search_contact():
    with open(humans, "r", encoding="utf-8") as humans_data:
        ...
        if contact_id not in data.get("contacts", []):
            print(f"\nNo contact with ID = {contact_id}!")
            contact_id = input("Please write contact ID or 'ex' for exit: ").lower()
            if contact_id == "ex":
                menu_start()
            else:
                table = []
...
def change_contact():
    ...
    contact = next(
        (c for c in data.get("contacts", []) if str(c.get("id")) == contact_id), None
    )
    ...
    fields = {"1": "name", "2": "telephone", "3": "specialization"}
    ...
    while True:
        ...
        if field_choice == "0":
            ...
            new_value = input(
                f"Please input new {fields[field_choice]} for {contact.get('name')}: "
            )
...
def delete_contact():
    ...
    contact_to_delete = next(
        (c for c in contacts if str(c.get("id")) == contact_id), None
    )
    ...
    confirm = input(
        f"Are you sure you want to delete the contact '{contact_to_delete.get('name')}'? (y/n): "
    )
    if confirm.lower() == "y":
